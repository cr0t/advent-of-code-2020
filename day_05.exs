defmodule Solution do
  ...
  defp normalize(input) do
    ...
    |> String.split("\n", trim: true)
    |> Enum.map(fn code ->
      [[_, row, col]] = Regex.scan(~r/^([FB]{7})([LR]{3})$/, code)
      [row: row, col: col]
    end)
  end
  ...
  defp find_row([], low, high), do: div(low + high, 2)
  ...
  defp find_col([], low, high), do: div(low + high, 2)
  defp find_col(["L" | tail], low, high), do: find_col(tail, low, div(low + high, 2))
  defp find_col(["R" | tail], low, high), do: find_col(tail, div(low + high, 2) + 1, high)
end
